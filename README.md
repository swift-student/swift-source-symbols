...
Status: Tree-sitter Swift, Ruby, Kotlin, and TypeScript/TSX extractors with a permanent declaration corpus and a runnable in-memory example.
...
The package foundation and CI are in place. Parser packaging and the documented backend decision inform the remaining support work.
...
The library product and importable module are both `SourceSymbols`. Add
`.package(url: "https://github.com/swift-student/swift-source-symbols.git", from: "0.1.0")`
to your SwiftPM dependencies and
`.product(name: "SourceSymbols", package: "swift-source-symbols")` to the consuming
target. For local development, use `.package(path: "../swift-source-symbols")`.
